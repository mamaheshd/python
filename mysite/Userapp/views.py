        ...
        if password_reset_form.is_valid():
            ...
            if associated_users.exists():
                for user in associated_users:
                    ...
                    try:
                        send_mail(subject, email, 'memaheshd@gmail.com' , [user.email], fail_silently=False)
                    except BadHeaderError:
                        return HttpResponse('Invalid header found.')
                    messages.success(request, 'A message with reset password instructions has been sent to your inbox.')
                    return redirect ("user_login") # go to login 
            messages.error(request, 'An invalid email has been entered.')
    password_reset_form = PasswordResetForm()
    return render(request=request, template_name="Userapp/password_reset_form.htm", context={"password_reset_form":password_reset_form})
